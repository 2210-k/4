...
<body>
    <div class="container">
        <header>
            ...
            <p class="description">Платформа для ролевых игр в реальном мире</p>
        </header>
    ...
    <script>
        ...
        function startEMSTimer() {
            ...
            // Устанавливаем случайное время до следующего вызова (7-10 минут)
            nextEmsCall = getRandomInt(7, 10) * 60 * 1000;
        ...
        function startMCHSTimer() {
            ...
            // Устанавливаем случайное время до следующего вызова (8-11 минут)
            nextMchsCall = getRandomInt(8, 11) * 60 * 1000;
        ...
        function startContractsTimer() {
            ...
            // Устанавливаем время до следующего обновления контрактов (10 минут)
            nextContractsUpdate = 10 * 60 * 1000;
            updateTimerDisplay('contracts-timer', nextContractsUpdate);
            
            contractsTimerInterval = setInterval(() => {
                nextContractsUpdate -= 1000;
                
                if (nextContractsUpdate <= 0) {
                    generateContracts();
                } else {
                    updateTimerDisplay('contracts-timer', nextContractsUpdate);
                }
